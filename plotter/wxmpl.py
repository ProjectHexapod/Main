...
def find_selected_axes(canvas, x1, y1, x2, y2):
    ...
    """
    axes = None
    bbox = Bbox(numpy.array[(x1, y1), (x2, y2)])
    ...
    b = Bbox(numpy.array([xymin, xymax]))
    xrange, yrange = get_bbox_lims(b.inverse_transformed(axes.transData))
    ...
    def leftButtonUp(self, evt, x, y):
        ...
        if axes is not None:
            ...
            if self.zoomEnabled:
                ...
                bbox = Bbox(numpy.array([(x0, y0), (x, y)]))
...
class VectorBuffer:
    ...
    """
    def __init__(self):
        self.data = numpy.zeros((16,), numpy.Float)
        self.nextRow = 0
    ...
    def append(self, point):
        ...
        if resize:
            self.data = numpy.zeros((nR,), numpy.Float)
...
class MatrixBuffer:
    ...
    """
    def __init__(self):
        self.data = numpy.zeros((16, 1), numpy.Float)
        self.nextRow = 0
    ...
    def append(self, row):
        ...
        """
        row = numpy.asarray(row, numpy.Float)
        nextRow = self.nextRow
        data = self.data
        nPts = row.shape[0]
        ...
        if resize:
            self.data = numpy.zeros((nR, nC), numpy.Float)
...
def make_bbox(X, Y):
    ...
    return Bbox(numpy.array(((x1,y1),(x2,y2))))
    ...
    def _update_channel(self, channel, zoomed):
        ...
        if line._wxmpl_empty_line:
            ...
            if line.get_transform() != axes.transData:
                ...
                x = numpy.array([a for (a, b) in xys])
                y = numpy.array([b for (a, b) in xys])
