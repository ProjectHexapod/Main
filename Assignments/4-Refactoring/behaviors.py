...
class PutFootOnGround:
    ...
    def update(self):
        ...
        if not self.isDone():
            self.current_foot_pos[Z] -= self.vel * time_sources.global_time.getDelta()
        return self.leg.jointAnglesFromFootPos(self.current_foot_pos)
...
class TrapezoidalFootMove:
    ...
    def update(self):
        remaining_vector = self.final_foot_pos - self.current_foot_pos
        if not self.done and not arraysAreEqual(self.getNormalizedRemaining(), self.dir):
	    self.done = True
            self.current_foot_pos = self.final_foot_pos
        if not self.isDone():
            ...
            self.current_foot_pos += self.dir * self.vel * delta
        return self.leg.jointAnglesFromFootPos(self.current_foot_pos)
