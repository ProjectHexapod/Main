...
class RotateFootAboutOrigin:
    ...
    def __init__(self, body_model, body_controller, leg_index, delta_angle, max_velocity, acceleration):
        ...
        self.body_coord = self.body_model.transformLeg2Body(self.leg_index,
                                                            self.leg_model.footPosFromLegState([self.limb_controller.getDesiredPosAngle(), 0]))  # self.leg_model.getShockDepth()]))
    ...
    def update(self):
        if not self.isDone():
            ...
            self.target_foot_pos = [self.radius * cos(self.target_angle),
                                    self.radius * sin(self.target_angle),
                                    self.init_height]
            
            if not sign(remaining_angle) == self.dir:
                self.done = True
                self.target_foot_pos = [self.radius * cos(self.delta_angle + self.init_angle),
