...
class RotateFootAboutOrigin:
    ...
    def update(self):
        if not self.isDone():
            ...
            if self.leg_index == 0:
                leg_coord = self.leg_model.getFootPos()
                hip_offset = self.body_model.getHipOffset(self.leg_index)
                rotated = rotateZ(leg_coord,hip_offset[2])
                print(leg_coord)
                print("rotated", rotated)
                body_coord = rotated+array([hip_offset[0],hip_offset[1],0])
                #print("body", norm(body_coord))
                #print("workedout position", body_coord)
                #print("transformed position", self.body_model.transformLeg2Body(0,leg_coord))
