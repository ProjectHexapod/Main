...
class RotateFootAboutOrigin:
    ...
    def __init__(self, body_model, leg_index, leg_model, limb_controller, delta_angle, max_velocity, acceleration):
        ...
        self.target_angle = arctan2(self.body_coord[1], self.body_coord[0])
    ...
    def update(self):
        if not self.isDone():
            ...
            self.body_coord = self.body_model.transformLeg2Body(self.leg_index, self.leg_model.getFootPos())
            self.target_angle = arctan2(self.body_coord[1], self.body_coord[0])
