...
class PutFootOnGroundTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_leg_model = mox.MockAnything()
        self.mock_leg_model.isFootOnGround().AndReturn(False)
        self.mock_leg_model.getFootPos().AndReturn([.1, 0., 0.])

    def testIsDone(self):
        mox.Replay(self.mock_leg_model)
        pfog = PutFootOnGround(self.mock_leg_model, 1)
        self.assertFalse(pfog.isDone())
        pfog.done = True
        mox.Verify(self.mock_leg_model)

    def testUpdateOnGround(self):
        self.mock_leg_model.isFootOnGround().AndReturn(True)
        self.mock_leg_model.jointAnglesFromFootPos([.1, 0, 0])
        mox.Replay(self.mock_leg_model)
        pfog = PutFootOnGround(self.mock_leg_model, 1)
        self.assertFalse(pfog.isDone())
        pfog.update()
...
class TrapezoidalFootMoveTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_leg_model = mox.MockAnything()
        self.mock_leg_model.getFootPos().AndReturn(array([.2, .2, .2]))
    ...
    def testIsDone(self):
        mox.Replay(self.mock_leg_model)
        trap = TrapezoidalFootMove(self.mock_leg_model,
                                   array([.2, .2, 0]), 10, .1)
    ...
    def testUpdateBecommingDone(self):
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, 0]))
        mox.Replay(self.mock_leg_model)
        trap = TrapezoidalFootMove(self.mock_leg_model,
                                   array([.2, .2, 0]), 10, 1)
        self.assertFalse(trap.isDone())
    ...
    def testLeadingEdge(self):
        ...
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .19]))
        mox.Replay(self.mock_leg_model)
        trap = TrapezoidalFootMove(self.mock_leg_model,
                                   array([.2, .2, 0]), 10, 1)
        global_time.updateDelta(.1)
    ...
    def testLeadingEdgeStrait(self):
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .19]))
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .17]))
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .14]))
        mox.Replay(self.mock_leg_model)
        trap = TrapezoidalFootMove(self.mock_leg_model,
                                   array([.2, .2, 0]), 10, 1)
        global_time.updateDelta(.1)
    ...
    def testPlateau(self):
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .19]))
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .18]))
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .17]))
        mox.Replay(self.mock_leg_model)
        trap = TrapezoidalFootMove(self.mock_leg_model,
                                   array([.2, .2, 0]), .1, 1)
    ...
    def testTriangleCase(self):
        ...
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .16]))
        mox.Replay(self.mock_leg_model)
    ...
    def testDoesntPassTargetFootPos(self):
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .1]))
        self.mock_leg_model.jointAnglesFromFootPos(ArraysEqual([.2, .2, .0]))
        mox.Replay(self.mock_leg_model)
