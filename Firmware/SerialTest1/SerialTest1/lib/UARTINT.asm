...
_UART_RX_ISR:
   ...
   ; Grab the byte, check if the parity bit is set to mark
   ; If it is, this is an address
   ; decrement and forward, figure out if we're selected
   ; If it's data and we're not selected, forward
   ; if it's data and we're selected, process
   push A
   ;push X
   ; Did we get a parity error (if so this is an address)
   mov A, REG[UART_RX_CONTROL_REG]
   and A, UART_RX_PARITY_ERROR
   jz  got_data
   got_address:
   
   
   call _UART_Stop
   mov  A,UART_PARITY_ODD
   call _UART_Start
   mov  A,REG[UART_RX_BUFFER_REG]    ; Read the received data
   mov  [_selected_address],A        ; Save the address
   dec  A						     ; Decrement the address before forwarding
   mov REG[UART_TX_BUFFER_REG],A     ; Forward the decremented address
   call _UART_Stop
   mov  A,UART_PARITY_EVEN
   call _UART_Start
   jmp resolution_complete
   got_data:
   ;cmp  [_selected_address],0x00     ; Are we selected?
   ;jz   we_are_selected
   we_are_not_selected:
